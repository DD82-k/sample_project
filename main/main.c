...
#define EXAMPLE_LCD_TOUCH_RST           (GPIO_NUM_6)
#define EXAMPLE_LCD_TOUCH_INT           (GPIO_NUM_7)
#define I2C_MASTER_FREQ_HZ              10 * 1000

#define EXAMPLE_LVGL_BUFFER_SIZE        EXAMPLE_LCD_H_RES * 50 * sizeof(lv_color_t)
...
static void example_lvgl_touch_cb(lv_indev_t *indev, lv_indev_data_t *data)
{
    ...
    void *touch_handle = lv_indev_get_user_data(indev);
...
static void example_lvgl_port_task(void *arg)
{
    ...
    ui_init();
    ESP_LOGI(TAG, "UI init done, entering LVGL loop");
...
void app_main(void)
{
    ...
    const spi_bus_config_t buscfg = ST7789_PANEL_BUS_SPI_CONFIG(EXAMPLE_PIN_NUM_LCD_PCLK, EXAMPLE_PIN_NUM_LCD_DATA0,
    ...
    const esp_lcd_panel_io_spi_config_t io_config = ST7789_PANEL_IO_SPI_CONFIG(EXAMPLE_PIN_NUM_LCD_CS, EXAMPLE_PIN_NUM_LCD_DC,
    ...
    size_t buffer_size_bytes = EXAMPLE_LCD_H_RES * 50 * sizeof(lv_color_t);
    ...
    /* Keep-alive: periodic I2C probe prevents CST816T from sleeping */
    while (1) {
        i2c_cmd_handle_t wake = i2c_cmd_link_create();
        i2c_master_start(wake);
        i2c_master_write_byte(wake, (0x15 << 1) | I2C_MASTER_WRITE, true);
        i2c_master_stop(wake);
        i2c_master_cmd_begin(0, wake, pdMS_TO_TICKS(50));
        i2c_cmd_link_delete(wake);
